...
public:
    ...
    public:
        ...
        void NoteMessage(const PayloadHeader & payloadHeader)
        {
            ...
            printf("NoteMessage ==> Message type %d\n", (int)payloadHeader.GetMessageType());

            if (payloadHeader.HasMessageType(SecureChannel::MsgType::CASE_Sigma1))
            {
                mSaw1 = true;
            }
            else if (payloadHeader.HasMessageType(SecureChannel::MsgType::CASE_Sigma2))
            {
                mSaw2 = true;
            }
            else if (payloadHeader.HasMessageType(SecureChannel::MsgType::CASE_Sigma3))
            {
                mSaw3 = true;
            }
            else if (payloadHeader.HasMessageType(SecureChannel::MsgType::StatusReport))
            {
                mSawStatus = true;
            }
            else
                                           ...
                                           TestCASESecurePairingDelegate & delegateCommissioner, bool expectSuccess = true)
{
    ...
    NL_TEST_ASSERT(inSuite,
                   ctx.GetExchangeManager().RegisterUnsolicitedMessageHandlerForType(SecureChannel::MsgType::CASE_Sigma1,
    ...
    // scope.
    NL_TEST_ASSERT(inSuite,
                   ctx.GetExchangeManager().UnregisterUnsolicitedMessageHandlerForType(SecureChannel::MsgType::CASE_Sigma1) ==
                       CHIP_NO_ERROR);
}
...
void CASE_SecurePairingEmptySigma1(nlTestSuite * inSuite, void * inContext)
{
    TestCASESecurePairingDelegate delegateCommissioner;
    TestCASESessionIPK pairingCommissioner;
...
static const nlTest sTests[] =
{
    ...
    NL_TEST_DEF("EmptySigma1", CASE_SecurePairingEmptySigma1),
