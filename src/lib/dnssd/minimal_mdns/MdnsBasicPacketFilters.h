...
public:
    ...
    FilterOutcome FilterBeforeEnqueue(const void * endpoint, const chip::Inet::IPPacketInfo & pktInfo, const chip::System::PacketBufferHandle & pktPayload) override
    {
        ...
        if (mNumQueuedMdnsPackets.load() >= mMaxQueuedPackets)
        {
            if (!mStopLogging)
            {
                ChipLogError(Discovery, "Dropped!");
                mStopLogging = true;
            }
    ...
    FilterOutcome FilterAfterDequeue(const void * endpoint, const chip::Inet::IPPacketInfo & pktInfo, const chip::System::PacketBufferHandle & pktPayload) override
    {
        ...
        --mNumQueuedMdnsPackets;
        if (mNumQueuedMdnsPackets.load() == 0)
        {
            if (mStopLogging)
            {
                mStopLogging = false;
                ChipLogError(Discovery, "Resolved!");
            }
        }
    ...
    void SetMaxQueuedPacketsLimit(size_t maxQueuedPackets) { mMaxQueuedPackets = maxQueuedPackets; }
