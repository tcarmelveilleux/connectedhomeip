...
class TC_OCC_3_2(MatterBaseTest):
    ...
    async def test_TC_OCC_3_2(self):
        ...
        diff_val = 12
        await self.write_single_attribute(attributes.HoldTime(diff_val), expect_success=True)
        ...
        if not has_type_pir or not has_pir_timing_attrib:
            ...
            diff_val = 11
            await self.write_single_attribute(attributes.PIROccupiedToUnoccupiedDelay(diff_val), expect_success=True)
        ...
        if not has_type_ultrasonic or not has_ultrasonic_timing_attrib:
            ...
            # write the new attribute value
            diff_val = 14
            await self.write_single_attribute(attributes.UltrasonicOccupiedToUnoccupiedDelay(diff_val), expect_success=True)
        ...
        if not has_type_contact or not has_contact_timing_attrib:
            ...
            # write the new attribute value
            diff_val = 9
            await self.write_single_attribute(attributes.PhysicalContactOccupiedToUnoccupiedDelay(diff_val), expect_success=True)
