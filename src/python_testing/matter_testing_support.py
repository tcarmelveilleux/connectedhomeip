...
class MatterBaseTest(base_test.BaseTestClass):
    ...
    async def write_single_attribute(self, attribute_value: object, endpoint_id: int=None, expect_success: bool=False) -> Status:
        ...
        if expect_success:
            asserts.assert_equal(write_result[0].Status, Status.Success, f"Expected write success for write to attribute {attribute_value} on endpoint {endpoint}")
