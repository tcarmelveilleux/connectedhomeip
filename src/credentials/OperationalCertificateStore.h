...
public:
    ...
    /**
     * @brief Returns true if a pending operational certificates exists and are active from a previous
     *        `AddNewTrustedRootCertForFabric`, `AddNewOpCertsForFabric` or `UpdateOpCertsForFabric`.
     */
    virtual bool HasPendingCertificates() const = 0;
     ...
     * @param certType - Type of certificate whose presence needs to be checked
     * @return true if there an active obtainable operational certificate of the given type for the given FabricIndex,
     *         false otherwise.
     */
    virtual bool HasCertificateForFabric(FabricIndex fabricIndex, CertificateType certType) const = 0;
     ...
     *        `UpdateOpCertsForFabric`.
     *
     * @param fabricIndex - fabricIndex for which to get the certificate
     * @param certType - type of certificate to get
     * @param outCertificate - buffer to contain the certificate obtained from persistent or tempoarary storage
     * @retval CHIP_NO_ERROR on success
     *
     * TODO: FINISH THE Method docs
     */
    virtual CHIP_ERROR GetCertificate(FabricIndex fabricIndex, CertificateType certType, const MutableByteSpan & outCertificate);
};
