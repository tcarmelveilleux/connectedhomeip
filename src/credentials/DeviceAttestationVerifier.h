...
constexpr size_t kMaxAttestationNonceSize = 32;
...
public:
    ...
    virtual AttestationVerificationResult
        VerifyAttestationInformation(const ByteSpan & attestation_info_buffer,
                                     const ByteSpan & attestation_challenge_buffer,
                                     const ByteSpan & attestation_signature_buffer,
                                     const ByteSpan & pai_cert_der_buffer,
                                     const ByteSpan & dac_cert_der_buffer) = 0;


    /**
     * @brief Obtain an attestation Nonce for the Device Attestation Procedure. If multiple
     *        device attestation procedures are supported by a Commissioner's implementation,
     *        it should be able to give a different Nonce each time and track attestation
     *        responses by embedded repeated nonce.
     * @param[inout] attestation_nonce_buffer Buffer to receive the nonce. Must be at least kMaxAttestationNonceSize.
     *
     * @returns CHIP_NO_ERROR on success, CHIP_ERROR_BUFFER_TOO_SMALL if `attestation_nonce_buffer`
     *          is too small, or CHIP_INTERNAL if obtaining a random nonce fails.
     */
    virtual CHIP_ERROR GetAttestationNonce(const MutableByteSpan & attestation_nonce_buffer) = 0;
};
