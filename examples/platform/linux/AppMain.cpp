...
void ChipLinuxAppMainLoop()
{
    ...
    // Global singleton storage based on KeyValueStore injection
    static chip::KvsPersistentStorageDelegate kvsPersistentStorageDelegate;
    chip::DeviceLayer::PersistedStorage::KeyValueStoreManager & kvsManager = DeviceLayer::PersistedStorage::KeyValueStoreMgr();
    VerifyOrDie(kvsPersistentStorageDelegate.Init(&kvsManager) == CHIP_NO_ERROR);
    initParams.persistentStorageDelegate = &kvsPersistentStorageDelegate;

    // Global singleton Group Data provider injection
    static chip::Credentials::GroupDataProviderImpl groupDataProvider;
    groupDataProvider.SetStorageDelegate(&kvsPersistentStorageDelegate);
